...
- (nullable id)pesdk_getValueForKeyPath:(nonnull NSString *)keyPath default:(nullable id)defaultValue;
...
+ (void)setWillPresentPhotoEditViewController:(CDV_PESDKWillPresentBlock)willPresentBlock {
  _willPresentPhotoEditViewController = willPresentBlock;
}

const struct CDV_IMGLY_Constants CDV_IMGLY = { .kErrorUnableToUnlock = @"E_UNABLE_TO_UNLOCK",
...
- (void)present:(CDVInvokedUrlCommand *)command {
  ...
  if (self.lastCommand == nil) {
    ...
    filepath = [filepath stringByReplacingOccurrencesOfString:@"imgly_asset:///"
    ...
    if (tempWithConfiguration != nil) {
      NSData *jsonTestData = [NSJSONSerialization dataWithJSONObject:tempWithConfiguration
    ...
    IMGLYMediaEditViewControllerBlock createMediaEditViewController = ^PESDKMediaEditViewController *_Nullable(
      ...
      PESDKPhotoEditViewController *photoEditViewController =
        [PESDKPhotoEditViewController photoEditViewControllerWithPhotoAsset:photoAsset
                                                              configuration:configuration
                                                             photoEditModel:photoEditModel];
      photoEditViewController.modalPresentationStyle = UIModalPresentationFullScreen;
      photoEditViewController.delegate = self;
      CDV_PESDKWillPresentBlock willPresentPhotoEditViewController = PESDKPlugin.willPresentPhotoEditViewController;
    ...
    id valueSerializationEnabled = [NSDictionary pesdk_getValue:withConfiguration
                                                valueForKeyPath:@"export.serialization.enabled"
                                                        default:@(NO)];
    id valueSerializationType = [NSDictionary pesdk_getValue:withConfiguration
                                             valueForKeyPath:@"export.serialization.exportType"
                                                     default:CDV_IMGLY.kExportTypeFileURL];
    id valueSerializationFile = [NSDictionary pesdk_getValue:withConfiguration
                                             valueForKeyPath:@"export.serialization.filename"
                                                     default:valueExportFile];
    id valueSerializationEmbedImage = [NSDictionary pesdk_getValue:withConfiguration
                                                   valueForKeyPath:@"export.serialization.embedSourceImage"
                                                           default:@(NO)];

    NSString *exportType = valueExportType;
    NSURL *exportFile = [CDVConvert CDV_IMGLY_ExportFileURL:valueExportFile withExpectedUTI:getUTI(configuration)];
    BOOL serializationEnabled = [valueSerializationEnabled boolValue];
    NSString *serializationType = valueSerializationType;
    NSURL *serializationFile = [CDVConvert CDV_IMGLY_ExportFileURL:valueSerializationFile withExpectedUTI:kUTTypeJSON];
    BOOL serializationEmbedImage = [valueSerializationEmbedImage boolValue];

    // Make sure that the export settings are valid
    if ((exportType == nil) || (exportFile == nil && [exportType isEqualToString:CDV_IMGLY.kExportTypeFileURL]) ||
        (serializationFile == nil && [serializationType isEqualToString:CDV_IMGLY.kExportTypeFileURL])) {
    ...
    configuration = [[PESDKConfiguration alloc] initWithBuilder:^(PESDKConfigurationBuilder *_Nonnull builder) {
      ...
      IMGLYConfigurationBlock configureWithBuilder = PESDKPlugin.configureWithBuilder;
...
- (void)photoEditViewControllerDidFinish:(PESDKPhotoEditViewController * _Nonnull)photoEditViewController result:(PESDKPhotoEditorResult * _Nonnull)result {
  ...
  if (self.serializationEnabled) {
    ...
    } else if ([self.serializationType isEqualToString:CDV_IMGLY.kExportTypeObject]) {
  ...
  if (error == nil) {
    ...
    NSString *message = [NSString stringWithFormat:@"Unable to export image or serialization: %@", error];
    CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:message];
    [self closeControllerWithResult:result];
  }
}
...
+ (nullable id)pesdk_getValue:(nullable NSDictionary *)dictionary
  ...
  return [dictionary pesdk_getValueForKeyPath:keyPath default:defaultValue];
}
...
+ (nullable CDV_IMGLY_ExportFileURL *)CDV_IMGLY_ExportFileURL:(nullable id)json
